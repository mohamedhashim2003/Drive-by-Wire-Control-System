...
class ThrottleGUI(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # FIX: add refresh button so user can rescan ports without restarting the app
        ...
        # Live Tuning Card
        tuning_card = self._card()
        tuning_layout = QGridLayout(tuning_card)
        tuning_layout.setSpacing(8)

        tuning_title = QLabel("LIVE PID TUNING")
        tuning_title.setFont(QFont("Consolas", 10, QFont.Bold))
        tuning_title.setStyleSheet(f"color: {TEXT_SEC}; letter-spacing: 2px;")
        tuning_layout.addWidget(tuning_title, 0, 0, 1, 2)

        # Kp Input
        tuning_layout.addWidget(self._stat_label("Kp:"), 1, 0)
        self.input_kp = QLineEdit("2.0")
        self.input_kp.setStyleSheet(f"background-color: {BG_DEEP}; border: 1px solid {BORDER}; color: {TEXT_PRI}; padding: 4px; border-radius: 4px;")
        tuning_layout.addWidget(self.input_kp, 1, 1)

        # Ki Input
        tuning_layout.addWidget(self._stat_label("Ki:"), 2, 0)
        self.input_ki = QLineEdit("0.05")
        self.input_ki.setStyleSheet(f"background-color: {BG_DEEP}; border: 1px solid {BORDER}; color: {TEXT_PRI}; padding: 4px; border-radius: 4px;")
        tuning_layout.addWidget(self.input_ki, 2, 1)

        # Kd Input
        tuning_layout.addWidget(self._stat_label("Kd:"), 3, 0)
        self.input_kd = QLineEdit("0.8")
        self.input_kd.setStyleSheet(f"background-color: {BG_DEEP}; border: 1px solid {BORDER}; color: {TEXT_PRI}; padding: 4px; border-radius: 4px;")
        tuning_layout.addWidget(self.input_kd, 3, 1)

        # Apply Button
        self.btn_tune = QPushButton("APPLY GAINS")
        self.btn_tune.setFont(QFont("Consolas", 10, QFont.Bold))
        self.btn_tune.setFixedHeight(36)
        self.btn_tune.clicked.connect(self._send_pid_gains)
        tuning_layout.addWidget(self.btn_tune, 4, 0, 1, 2)

        right_col.addWidget(tuning_card)
        ...
        # FIX: addLegend must be called before plot() or pyqtgraph creates a duplicate legend
    ...
    def _on_disconnect(self):
        # FIX: clear serial_thread reference so _toggle_connection doesn't see a dead thread
        self.serial_thread = None
        self.btn_connect.setText("CONNECT")
        self.status_dot.setStyleSheet("color: #FF4D6D; font-size: 18px;")
    ...
    def _on_data(self, pedal, tps, error, pwm, mode):
        ...
        # FIX: sync mode buttons if Arduino reports a different mode (prevents silent desync)
        if mode != self.current_mode:
            self.current_mode = mode
            for i, btn in enumerate(self.mode_buttons):
                btn.setChecked(i == mode)
            self._update_mode_buttons()
            self.mode_desc_label.setText(MODE_DESC[MODE_NAMES[mode]])
    ...
    def _send_pid_gains(self):
        try:
            kp = float(self.input_kp.text().strip())
            ki = float(self.input_ki.text().strip())
            kd = float(self.input_kd.text().strip())

            # FIX: reject negative/zero Kp — matches the Arduino-side validation
            ...
            if self.serial_thread and self.serial_thread.isRunning():
                self.serial_thread.send(packet)
                self.btn_tune.setText("GAINS APPLIED!")
                QTimer.singleShot(1500, lambda: self.btn_tune.setText("APPLY GAINS"))
        except ValueError:
            self.btn_tune.setText("INVALID INPUT")
            QTimer.singleShot(1500, lambda: self.btn_tune.setText("APPLY GAINS"))
